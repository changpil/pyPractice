...
def swapNodes(indexes, queries):
    ...
    for level in queries:
        swapNode(root, level -1)
        inorderTraversal(root, store)
        results.append(store.copy())
        store.clear()
    return results
def swapNode(node, level):
    if not node:
        return
    if level == 0:
        tmp = node.left
        ...
        node.right = tmp
        return
    swapNode(node.left , level -1)
    swapNode(node.right, level - 1)
...
def buildTree(indexes):
    ...
    i = 0
    while i < len(indexes):
        left = Node(indexes[i][0]) if indexes[i][0] != -1 else None
        right = Node(indexes[i][1]) if indexes[i][1] != -1 else None
        if left:
            queue.append(left)
        if right:
            queue.append(right)
...
def inorderTraversal(root, store):
    if root.left:
