...
class UndergroundSystem:
    ...
    def checkIn(self, id: int, stationName: str, t: int) -> None:
        self.tracking[id] = (stationName, t)

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        dep_s, dep_t = self.tracking[id]
        total , count = self.averageTime[(dep_s, stationName)]
        total += (t - dep_t)
        count += 1
        self.averageTime[(dep_s, stationName)] = (total, count)

    def getAverageTime(self, startStation, endStation):
        total , count = self.averageTime[(startStation, endStation)]
        if count != 0:
            return total/count
        else:
            return None
